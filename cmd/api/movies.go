...
func (app *application) updateMovieHandler(w http.ResponseWriter, r *http.Request) {
	// 1. Extract the movie ID from the URL.
	id, err := app.readIDParam(r)
	if err != nil {
		app.notFoundResponse(w, r)
		return
	...
	// 2. Fetch the corresponding movie record from the db.
	movie, err := app.models.Movies.Get(id)
	if err != nil {
		switch {
		case errors.Is(err, data.ErrRecordNotFound):
	...
	// 6. Call the Update() method to store the udpated movie record in our db.
	...
	// 7. Write the updated move data in a JSON response using the app.writeJSON()
	err = app.writeJSON(w, http.StatusOK, envelope{"movie": movie}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
