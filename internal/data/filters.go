...
// calculateMetdata calculates the appropriate pagination metadata values given
// the total number of records, current page, and page size values. The last
// page value is calculated using the math.Ceil() function, which rounds up a
// float to the nearest integer. So, for example, if there were 12 records in
// total and a page size of 5, the last page value would be math.Ceil(12/5) = 3.
func calculateMetdata(totalRecords, page, pageSize int) Metadata {
	if totalRecords == 0 {
		return Metadata{}
	}
